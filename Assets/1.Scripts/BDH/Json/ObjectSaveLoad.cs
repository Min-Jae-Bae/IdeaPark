...
public class ObjectSaveLoad : MonoBehaviour
{
    ...
    void Update()
    {
        //3번키 누르면 랜덤한 모양, 크기, 위치, 회전 이 된 오브젝트 만들자.
        ...
        // 4번키 누르면 objectList 의 정보를 json 으로 objectInfo.txt 형태로 저장
        if (Input.GetKeyDown(KeyCode.Alpha4))
        {
            List<SaveInfo> saveInfoList = new List<SaveInfo>();

            // objectList 를 기반으로 저장할 정보
            ...
            // key : data , value : saveInfoList 값으로 셋팅. 
        ...
        //5번키 누르면 objectInfo.txt 에서 데이터를 읽어서 오브젝틀 만들자.
    ...
    // 호출될 때마다 X축으로 5 간격으로 Sphere가 생성
    void CreateShere()
    {
